    ...
    for (let itemIndex in itemType.data) {
      ...
      if (category) {
        if (seenIds[itemType.type].has(item.id)) {
          warnings.push(
            `Warning: Duplicate ID "${item.id}" found for ${itemType.type.slice(0, -1)} at index "${itemIndex}".`,
          );
          continue;
        }
        ...
        warnings.push(
          `Warning: Category "${item.category}" not found for ${itemType.type.slice(0, -1)} with ID "${item.id}" and index "${itemIndex}".`,
        );
      }
    }
  }
...
const main = (filePath = "entry.js") => {
  ...
  const { TP_PLUGIN_CATEGORIES, actions, states, events, connectors, TP_PLUGIN_INFO, TP_PLUGIN_SETTINGS } = script();
